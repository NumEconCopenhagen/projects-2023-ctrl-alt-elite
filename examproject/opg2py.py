...
class ProfitClass():
    ...
    def H_Delta(self, K):
        ...
        for t in range(120):
            val.kappa = np.exp(val.rho * np.log(kappa_prev) + epsilon[t])
        ell_ast = ((1 - val.eta) * val.kappa / val.w)**(1 / val.eta)
        ...
        profit = val.kappa * ell**(1 - val.eta) - val.w * ell - (ell != ell_prev) * val.iota
        h += val.R**(-t) * profit

        # Update variables for the next period
        kappa_prev = val.kappa
        ell_prev = ell
